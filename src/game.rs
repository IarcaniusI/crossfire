...
impl Game {
    ...
    pub fn render(&mut self, gl: &mut GlGraphics, args: &RenderArgs) {
        ...
        if !self.paused && !self.game_over {
            self.enimies_decision();
            self.logic();

            if self.hero.lives <= 0 {
                self.game_over = true;
                self.game_win = false;
            } else if self.enimies.len() <= 0 {
                self.game_over = true;
                self.game_win = true;
            }
        }
...
//        self.counter += 1;
        ...
        let point_num = self.point_num;
        let crash_num = self.crash_num;

        let game_over = self.game_over;
        let game_win = self.game_win;
        let paused = self.paused;

        let blocks = &self.blocks;
        let enimies = &self.enimies;
        let free_bullets = &self.free_bullets;
        let hero = &self.hero;
